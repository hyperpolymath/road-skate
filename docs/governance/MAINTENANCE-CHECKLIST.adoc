...
- [ ] Scan for unfinished markers: `PENDING`, `FIXME`, `XXX`, `HACK`, `STUB`, `PARTIAL`.
- [ ] If Idris is present, scan unsoundness markers: `b_me`, `a_total`.
...
- [ ] Remove or archive stale work:
  - close out completed PENDING/STUB/PARTIAL items
...
```bash
~/Desktop/run-maintenance.sh --repo /absolute/path/to/repo --output /home/user/maintenance-report.json
jq . /home/user/maintenance-report.json
```
...
"$PANIC_BIN" assail "$REPO" --output /home/user/assail.json --output-format json --quiet
jq -r '.weak_points | length' /home/user/assail.json
jq -r '.weak_points[] | "\(.severity)|\(.location)|\(.description)"' /home/user/assail.json
```

```bash
# If repo has production-only source builder, prefer this for baseline checks:
./scripts/ci/build-panic-assail-source.sh /home/user/panic-src
"$PANIC_BIN" assail /home/user/panic-src --output /home/user/assail-prod.json --output-format json --quiet
```
...
Repo: `/home/user/eclexia-releaseprep` (branch `release-prep`, base `533ec9e9447f374135cc9e2e81021624ddb3c0ad`)
...
- [x] Removed Idris `b_me` usage from ABI wrappers.
...
- [x] PENDING/FIXME/STUB/PARTIAL scan run on active code paths.
...
- Panic report: `/home/user/eclexia-panic-attack.json` (0 weak points)
- Final quality gate log: `/home/user/eclexia-quality-gate.log` (plus post-change reruns via terminal sessions)
...
4. Run PENDING/FIXME/stub scan and address relevant items.
